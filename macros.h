...
// these 'min'/'max' somehow conflict with GCC version of C++ STL: GNU ISO C++ Library, "stl_algobase.h"
//#ifndef __cplusplus
#if 0
#ifndef max
...
// don't use 'j', use 'ii' instead (it gives sensitively better readability (rather than just looks funny :) ))
...
//#define FOREACH(_id, _arr) for (decltype((_arr).begin()) _id = (_arr).begin(); (_id) != (_arr).end(); (_id)++)
// this version is insensitive to occasional change of '_arr'
// *if give a member-by-value for it, it cannot be compiled on MSVC 6.0 :  error C2654: 'my_array' : attempt to access member outside a member function
// this can be avoided by giving some nonmember :  a reference or a dereferenced pointer
...
// it has became too much classic - don't write such a long string for its name; 'SA()' is completely enough for this importantiest macro
...
// 2023-04-12; useless though (a template in its instance doesn't throw that away and reports an error (that's not what I expected))
#define SE typedef char _static_error_t[0]
